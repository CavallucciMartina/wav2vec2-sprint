...
#Evaluation Concat Dataset
test_dir = "../concat_cm/concat_dataset_cor_test_faiss/" #TO CHANGE!
file_transcripts = "txt/sentence.txt"
...
print("#### Loading dataset (concat test")
cmv_concat_test = create_hug_dataset(test_dir)
...
total_wer=0
total_cer=0
for index, batch in enumerate(cmv_concat_test):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        speech_array, sampling_rate = librosa.load(batch["path"], sr=16_000)
    batch["speech"] = speech_array
...
total_wer /= len(cmv_concat_test)
total_cer /= len(cmv_concat_test)

with open(f"evaluation_bands_concat_test_jonatas.txt", "w") as f:
    f.write("Common Voice Concat Dataset Evaluation with jonatasgrosman/wav2vec2-large-xlsr-53-italian on test set \n")
    ...
    f.write(f"Dataset Len: {len(cmv_concat_test)}\n")