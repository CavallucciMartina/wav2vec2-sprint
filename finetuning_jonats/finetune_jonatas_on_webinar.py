...
MODEL_ID = "jonatasgrosman/wav2vec2-large-xlsr-53-italian" #Change  to new model fine-tuned (path)
...
def create_hug_dataset(split_train):
    ...
    for speaker in speakers:
        ...
        #Get all audios from speaker
        list_mp3 = listdir(path_to_audio_dir)
        audio_dict = {mp3.split("/")[-1].split(".")[0]: mp3 for mp3 in list_mp3}

        #Get all transcripts for speaker
        with open(f"{path_to_speaker}{file_transcripts}", 'r') as f: 
            content = f.read()
        ...
        #sample = {}
        #field_values = len(data_fields) * ['0']
        #sample = {key: value for key, value in zip(data_fields, field_values)}

        for k, v in labels_dict.items():
            if k != "":
                dict_dataset['sentence'].append(v)
                dict_dataset['path'].append(f"{path_to_audio_dir}{audio_dict[k]}")
                #speech_array, sampling_rate = sf.read(new_path)
                #dict_dataset["duration"] = len(speech_array) / sampling_rate
                #It's necessary?

    tot_len = len(dict_dataset["path"])
    n_data_train = int((tot_len*split_train)/100)
    print(f"N DATA TRAIN webinar: {n_data_train}, N DATA VAL webinar: {tot_len-n_data_train}")
...
hug_dataset_train, hug_dataset_val = create_hug_dataset(split_train=80)
...
training_args = TrainingArguments(
            output_dir='./finetuning_jonatas_on_webinar/checkpoints',
            do_train=True,
            do_eval=True,
...
try:
    ...
    trainer.train()
    #trainer.train(path_checkpoint) #change with path of model
    print("#### Save model")
    trainer.save_model('./finetuning_jonatas_on_webinar/final')
except Exception as e:
    with open('./finetuning_jonatas_on_webinar/error.txt', 'w') as f:
        f.write(str(e))
    print(str(e))